...
public class AdvancedSkyboxPanel extends PluginPanel
{
    ...
    private final JComboBox<AdvancedSkyboxConfig.VideoAlignment> alignmentComboBox = new JComboBox<>(AdvancedSkyboxConfig.VideoAlignment.values());
    ...
    public AdvancedSkyboxPanel(AdvancedSkyboxPlugin plugin)
    {
        this.plugin = plugin;
        ...
        JLabel title = new JLabel("Advanced Skybox", SwingConstants.CENTER);
        title.setAlignmentX(Component.LEFT_ALIGNMENT);
        title.setMaximumSize(new Dimension(PANEL_WIDTH, 22));

        JLabel note = new JLabel("<html><body style='text-align:center; width:180px;'>Video-only local frame playback. Use captioned frames for shows or dialogue-heavy media.</body></html>", SwingConstants.CENTER);
        note.setAlignmentX(Component.LEFT_ALIGNMENT);
        note.setMaximumSize(new Dimension(PANEL_WIDTH, 48));

        JButton chooseFolderButton = fullWidthButton("Select Frame Pack Folder");
        chooseFolderButton.addActionListener(e -> plugin.choosePackFolder());

        JPanel transportPanel = new JPanel(new GridLayout(1, 3, 4, 0));
        ...
        JButton playButton = thirdWidthButton("Play");
        playButton.addActionListener(e -> plugin.resumeMedia());

        JButton pauseButton = thirdWidthButton("Pause");
        pauseButton.addActionListener(e -> plugin.pauseMedia());

        JButton restartButton = thirdWidthButton("Restart");
        restartButton.addActionListener(e -> plugin.restartMedia());
        ...
        {
            if (!updatingControls && alignmentComboBox.getSelectedItem() instanceof AdvancedSkyboxConfig.VideoAlignment)
            {
                plugin.setVideoAlignment((AdvancedSkyboxConfig.VideoAlignment) alignmentComboBox.getSelectedItem());
            }
        });
    ...
    public void updateStatus()
    {
        ...
        {
            folder = "No frame pack folder selected.";
        }
    ...
    private JButton thirdWidthButton(String text)
    {
        JButton button = new JButton(text);
        button.setFocusable(false);
        button.setPreferredSize(new Dimension((PANEL_WIDTH - 8) / 3, 24));
        button.setMinimumSize(new Dimension(40, 24));
        return button;
    }
